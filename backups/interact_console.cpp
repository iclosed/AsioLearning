...
void console_put(std::string msg) {
	std::cout << "\r                     \r";
	std::cout << msg << std::endl << "> ";
}

void timer_fire(asio::error_code ec, asio::steady_timer* tt) {
	console_put("nqbioagozige");
	tt->expires_after(std::chrono::milliseconds(280));
	tt->async_wait(std::bind(&timer_fire, std::placeholders::_1, tt));
}
...
int main() {
	...
	asio::steady_timer tt(io_context, std::chrono::seconds(1));
	tt.async_wait(std::bind(&timer_fire, std::placeholders::_1, &tt));
